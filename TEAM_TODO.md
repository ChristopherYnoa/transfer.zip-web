...
Scope: ship a credible WeTransfer-style team plan. Owners/Admins manage seats and invites; team members share branding and (at least see) each other's transfers; billing reflects reality. Skipping enterprise-only stuff (SSO/SAML/LDAP, audit-trail compliance exports) — not the bar we're shooting for yet.
...
- [ ] **Transfers are not team-scoped.** `Transfer.author` is a single user; `listTransfersForUser` only returns the caller's own. Team members can't see, manage, or hand off each other's transfers. Decide & implement: add `Transfer.team` (denormalized at creation), and let team members at least *view* team transfers. Admins/Owner should be able to delete or extend any team transfer.
...
- [ ] **Storage limits are ambiguous and probably wrong.** Pricing says 500GB *per seat* but `User.getStorage()` just calls `getMaxStorageForPlan(this.getPlan())` — each member independently gets 500GB regardless of seat count. Decide: pooled team storage (preferred), or per-seat enforced. Implement aggregation across `team.users` and clear messaging in the UI.
...
- [ ] **Invite re-send / reminder.** Pending invite expires after 7d (TTL on `TeamInvite`) with no UX. Add a "Resend" button in the pending-invite list and optionally a reminder email at day 3 or 6.
