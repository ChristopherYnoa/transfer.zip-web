...
- **Do not pass `className` to shadcn primitives to "fix" their spacing.** They already have the right padding, margin, gap, and alignment baked in. Adding `mt-*`, `space-*`, `gap-*`, `flex`, etc. to `DialogFooter`, `DialogHeader`, `DialogContent`, `Button`, etc. visibly breaks the design. If you need outer layout, wrap them in your own `<div>`. **`DialogFooter` in particular: never add a `className` to it.**
...
- **`<GenericPage>` is the default dashboard page shell** ([components/dashboard/GenericPage.js](next/src/components/dashboard/GenericPage.js)). Every page under `(app)/app/*` wraps in it — don't build a custom page chrome. It provides the white `DashH2` title, the top-right `side` slot (for header buttons or `text + button` combos like "5/10 seats used" + "Add user"), and consistent vertical rhythm. Use `titleComponent` only when the title needs custom interactivity (e.g. inline rename — see [TeamNameEditor.js](next/src/app/(app)/app/(admin)/admin/TeamNameEditor.js)). Don't pass `className` to override its spacing, and don't add `text-sm`/`text-xs` to content inside `side` — it inherits the base size that matches every other dashboard page header.
